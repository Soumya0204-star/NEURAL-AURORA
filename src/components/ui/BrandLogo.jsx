    ...
    >
      ...
      {connections.map(([i, j]) => (
        <motion.line
          ...
          strokeOpacity={0.1}
          strokeWidth={0.5 * s}
          animate={shouldReduceMotion ? undefined : { strokeOpacity: [0.06, 0.15, 0.06] }}
          transition={shouldReduceMotion ? { duration: 0 } : { duration: 3 + (i + j) % 3, repeat: Infinity, ease: 'easeInOut', delay: (i + j) * 0.2 }}
        />
      ))}
      ...
      <g stroke="#5DCAA5" strokeOpacity={0.2} strokeWidth={0.7 * s}>
        {[0, 1, 2, 3, 4, 5, 6].map((i) => (
          <motion.line
            ...
            animate={shouldReduceMotion ? undefined : { strokeOpacity: [0.15, 0.35, 0.15] }}
            transition={shouldReduceMotion ? { duration: 0 } : { duration: 3, repeat: Infinity, ease: 'easeInOut', delay: neuralNodes[i].delay }}
          />
        ))}
      </g>
      ...
      <motion.circle
        ...
        fill="url(#orbFill)"
        fillOpacity={0.35}
        animate={shouldReduceMotion ? undefined : { r: [10 * s, 11.5 * s, 10 * s] }}
...
export function BrandLogo({ size = 'default', showWordmark = true, asLink = false, onClick }) {
  ...
  const content = (
    <motion.div
      ...
      onClick={handleClick}
      style={shouldReduceMotion ? { perspective: 800 } : { rotateX, rotateY, perspective: 800 }}
  ...
  if (asLink && !onClick) {
    return (
      <a href="/" onClick={(e) => { e.preventDefault(); window.location.href = '/'; window.scrollTo({ top: 0, behavior: shouldReduceMotion ? 'auto' : 'smooth' }) }}>
        {content}
      </a>
    )
  }
...
export function BrandLogoFull({ className = '' }) {
  ...
  return (
    <motion.div
      ...
      transition={shouldReduceMotion ? { duration: 0 } : { duration: 1, ease: [0.16, 1, 0.3, 1] }}
